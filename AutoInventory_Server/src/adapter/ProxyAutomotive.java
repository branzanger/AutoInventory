...
public abstract class ProxyAutomotive {
	...
	public String listAutos() {
		...
		for (Automobile auto : a1.values()) {
			response.append(auto.toString()+"\n");
		}
		return response.toString();
	};
	
	public Automobile getAuto(String carName) {
		Automobile a = a1.get(carName);
		return a;
	};
	
	public String addAuto(Properties props) {
		Automobile auto = null;
		try {
			FileIO file = new FileIO();
			auto = file.readAuto(props);
			if (auto !=null)
				a1.put(auto.getName(), auto);	
			else
				throw new AutoException(81);
		...
		String response = auto.getName()
				+ " has been added to available automobiles. Press Enter to continue.\n";
		return response;
	}
	
	public String configAuto(Properties props) {
		
		String carName = null;
		try {
			FileIO file = new FileIO();
			carName = file.configAuto(props);
		}
		catch (AutoException ex) {
			ex.printStackTrace();
			return ex.fix();
